...
{ The vorbisfile callbacks for Object Pascal PStreams }
...
begin
  ...
  try
    result := Int64(PStream(datasource).Read(ptr, size * nmemb)) div Int64(size);
  except
    result := 0; { Assume nothing was read. No way to be sure of PStreams }
  end;
end;
...
begin
  try
    case whence of
      SEEK_CUR: PStream(datasource).Seek(offset, spCurrent);
      SEEK_END: PStream(datasource).Seek(offset, spEnd);
      SEEK_SET: PStream(datasource).Seek(offset, spBegin);
    end;
    result := 0;
  except
    result := -1;
...
begin
  try
    result := PStream(datasource).Position;
  except
    result := -1;
  end;
end;
